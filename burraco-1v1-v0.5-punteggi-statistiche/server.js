...
function stateFor(room, player) {
  ...
  return {
    ...
    nextHandAt: room.nextHandAt || null,
...
function finishHand(room, closingPlayer) {
  ...
  if (topScore >= (room.targetScore || 1005) && leaders.length === 1) {
    ...
    room.nextHandAt = Date.now() + 6000;
    setEvent(room, 'hand-end', closingPlayer, `Mano ${room.handNumber} terminata. Nuova mano tra pochi secondi.`, ['end']);
    setTimeout(() => {
      const current = rooms.get(room.code);
      if (!current || current.status !== 'hand-ended') return;
      startHand(current, false);
      emitRoom(current);
    }, 6000);
  }
}
...
io.on('connection', socket => {
  ...
  socket.on('leave-game', (ack = () => {}) => {
    ...
    if (!room.started && room.players.length === 1) {
      rooms.delete(code);
    } else if (room.status === 'playing' && opponent) {
      room.status = 'match-finished';
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Burraco 1v1 v0.5.0 avviato sulla porta ${PORT}`);
});
