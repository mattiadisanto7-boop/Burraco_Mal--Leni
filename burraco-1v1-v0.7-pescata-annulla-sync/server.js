...
function stateFor(room, player) {
  ...
  return {
    ...
    you: {
      ...
      recentlyDrawnCardIds: [...(player.recentlyDrawnCardIds || [])],
    ...
    canUndoLastMove: Boolean(room.turnUndo && room.turnUndo.playerId === player.id && room.turnPlayerId === player.id && room.status === 'playing' && room.turnStage === 'play'),
    needsUndoToAvoidBlock: Boolean(player.hand.length === 1 && player.potTaken && !playerHasBurraco(player)),
...
function saveTurnUndo(room, player, label) {
  room.turnUndo = {
    playerId: player.id,
    label,
    snapshot: {
      hand: deepClone(player.hand),
      melds: deepClone(player.melds),
      potTaken: player.potTaken,
      stats: deepClone(player.stats),
      pots: deepClone(room.pots),
      recentlyDrawnCardIds: [...(player.recentlyDrawnCardIds || [])]
    }
  };
}

function restoreTurnUndo(room, player) {
  const undo = room.turnUndo;
  if (!undo || undo.playerId !== player.id) return false;
  player.hand = deepClone(undo.snapshot.hand);
  player.melds = deepClone(undo.snapshot.melds);
  player.potTaken = undo.snapshot.potTaken;
  player.stats = deepClone(undo.snapshot.stats);
  player.recentlyDrawnCardIds = [...(undo.snapshot.recentlyDrawnCardIds || [])];
  room.pots = deepClone(undo.snapshot.pots);
  room.turnUndo = null;
  return true;
}
...
io.on('connection', socket => {
  ...
  socket.on('draw-deck', (ack = () => {}) => {
    ...
    player.recentlyDrawnCardIds = [card.id];
    player.stats.draws++;
    room.turnUndo = null;
    room.turnStage = 'play';
  ...
  socket.on('play-meld', ({ cardIds }, ack = () => {}) => {
    ...
    saveTurnUndo(room, player, 'calata');
  ...
  socket.on('add-to-meld', ({ meldId, cardIds }, ack = () => {}) => {
    ...
    saveTurnUndo(room, player, 'aggiunta');
    removeCards(player, picked.cards);
  ...
  socket.on('undo-last-move', (ack = () => {}) => {
    const { room, player } = roomAndPlayer(socket);
    if (!ensureCanAct(room, player, ack)) return;
    if (room.turnStage !== 'play') return ack({ ok: false, error: 'Non c’è una mossa da annullare in questo momento.' });
    if (!room.turnUndo || room.turnUndo.playerId !== player.id) {
      return ack({ ok: false, error: 'Non c’è una mossa precedente da annullare.' });
    }

    const label = room.turnUndo.label || 'mossa';
    if (!restoreTurnUndo(room, player)) return ack({ ok: false, error: 'Non riesco ad annullare la mossa.' });
    setEvent(room, 'undo', player, `${player.name} ha annullato l’ultima ${label}.`, []);
    ack({ ok: true });
    emitRoom(room);
  });

  socket.on('sync-state', (ack = () => {}) => {
    const { room, player } = roomAndPlayer(socket);
    if (!room || !player) return ack({ ok: false, error: 'Partita non trovata. Prova a rientrare con il codice stanza.' });
    player.connected = true;
    player.socketId = socket.id;
    socket.emit('game-state', stateFor(room, player));
    ack({ ok: true });
  });
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`Burraco 1v1 v0.7.0 avviato sulla porta ${PORT}`);
});
